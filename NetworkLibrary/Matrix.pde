  ...
  void dActivation() {
    for (int i = 0; i < rows; i++) {
      for (int j = 0; j < cols; j++) {
        ...
        matrix[i][j] = 1.0 - (matrix[i][j] * matrix[i][j]);
      }
    }
  }
  
  Matrix dActivation(Matrix m) {
    Matrix result = new Matrix(m.rows, m.cols);
    for (int i = 0; i < m.rows; i++) {
      for (int j = 0; j < m.cols; j++) {
        //result.matrix[i][j] = m.matrix[i][j] * (1 - m.matrix[i][j]);
        result.matrix[i][j] = (1.0 - (m.matrix[i][j] * m.matrix[i][j]));
      }
    }
    return result;
  }
    ...
    if (a.length == b.length && a[0].length == b[0].length) {
      for (int i = 0; i < b.length; i++) {
        for (int j = 0; j < b[i].length; j++) {
          c[i][j] = sq(a[i][j] - b[i][j]) * 1/2;
        }
      }
    } else {
      println("ERROR: ERROR CALCULATION INCOMPATIBLE");
    }
    return c;
  }

  Matrix squaredError(Matrix a, Matrix b) {
    Matrix result = new Matrix(a.rows, a.cols);
    result.setVals(squaredError(a.matrix, b.matrix));
    return result;
  }
}
